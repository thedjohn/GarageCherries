...
function mockSuccessfulPostChain(privacyLevels: string[] = ['PUBLIC_TO_EVERYONE', 'SELF_ONLY']) {
  (fetch as any)
    // token refresh
    .mockResolvedValueOnce({ ok: true, json: async () => ({ access_token: 'access-token' }) })
    // creator_info/query
    .mockResolvedValueOnce({ ok: true, json: async () => ({ error: { code: 'ok' }, data: { privacy_level_options: privacyLevels } }) })
...
describe('postListingReelToTikTok', () => {
  ...
  it('returns false when querying creator info fails', async () => {
    (fetch as any)
      .mockResolvedValueOnce({ ok: true, json: async () => ({ access_token: 'access-token' }) })
      .mockResolvedValueOnce({ ok: false, json: async () => ({ error: { code: 'not_ok' } }) });
    const result = await postListingReelToTikTok(LISTING, VIDEO_URL);
    expect(result).toBe(false);
  });
  ...
  it('posts successfully and returns true, defaulting to PUBLIC_TO_EVERYONE', async () => {
    mockSuccessfulPostChain();
    const result = await postListingReelToTikTok(LISTING, VIDEO_URL);
    expect(result).toBe(true);

    const initCall = (fetch as any).mock.calls[3];
    ...
    const uploadCall = (fetch as any).mock.calls[4];
    expect(uploadCall[0]).toBe('https://upload.example.com/session-1');
    expect(uploadCall[1].method).toBe('PUT');
  });

  it('respects an explicit privacyLevel override', async () => {
    mockSuccessfulPostChain(['PUBLIC_TO_EVERYONE', 'SELF_ONLY']);
    await postListingReelToTikTok(LISTING, VIDEO_URL, 'SELF_ONLY');
  ...
  it('tags a Corvette as #SportsCar', async () => {
    ...
    const body = JSON.parse((fetch as any).mock.calls[3][1].body);
