...
- [x] **Password reset email redirect fixed** — Supabase's Redirect URLs allow-list was missing `/account/reset-password` and `/dealer/login`, so reset emails silently fell back to the bare homepage instead of the reset form; fixed by adding `https://garagecherries.com/**` and `https://www.garagecherries.com/**` wildcard entries in Supabase Dashboard → Authentication → URL Configuration (dashboard-only change, fixed 2026-07-10)
...
- [x] **GitHub Actions CI** — `.github/workflows/ci.yml`; runs `tsc --noEmit` + `npm test` (Vitest unit tests) on every push to `main` and on PRs; Node 24; branch protection requires `check` job to pass (added 2026-07-09). Was actually red the whole day — `tests/unit/api.listings.patch.test.ts` had a fake `Request` cast that didn't satisfy the `NextRequest` param type; GitHub auto-bypassed the failing check due to admin permissions. Fixed 2026-07-09 (cast changed to `as unknown as NextRequest`) — the `check` job should now genuinely pass rather than being bypassed
