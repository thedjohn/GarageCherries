...
| ~~**Buyer Inquiry History**~~ | ✅ Already covered — buyers already see everything they've sent to sellers via the `/account` Messages tab (`conversations`/`messages` tables, the live two-way system behind "Message Seller"). A separate `/account/inquiries` page would show the `inquiries` table, which has had no live writer since that button switched to `conversations` — building it would show buyers nothing new. Confirmed 2026-07-14 while rewiring the dealer-facing consumers of the same stale data (see 2026-07-14 changelog entry) | — |
...
2. **Delete `app/sell/SellForm.tsx`** — dead file, never imported; `/sell` renders `SellClient.tsx` exclusively
3. **Decide on dealer self-serve signup** — current apply-and-wait model may be intentional (vetting quality), but if faster growth is the goal, self-serve + Stripe removes the bottleneck
4. **Restore inspection-affiliate button** — once Lemon Squad agreement is confirmed
5. **Submit event pages to Google** — use URL Inspection in Search Console to request indexing for individual `/events/[slug]` pages; rich Event results will appear once Google crawls the JSON-LD
6. **Add actual sale price to `/sold`** — optional "Sold for $X" field on Mark as Sold flow; deferred until enough sold listings accumulate to make it worthwhile
7. **Decide whether to fully delete the orphaned `inquiries` table/route** — `app/api/inquire/route.ts` has had no live caller since "Message Seller" moved to `conversations`; left in place 2026-07-14 for now (harmless, unused) rather than deleted, in case there's a reason to keep the audit trail or revive it as a separate lightweight contact-form path alongside full conversations
