...
- [x] **Sign in with Facebook** on `/account/login` and `/account/signup` (`components/FacebookSignInButton.tsx`) — mirrors the Google sign-in implementation exactly (same `profiles`-seeding gate in `/auth/callback`, now keyed on `provider` being `google` or `facebook`; same `?promo=`/`?return=` handling); buyer-only, same reasoning as Google. **Facebook app is still Unpublished (Development mode)** — only Admins/Developers/Testers on the Meta app (App ID `1342998511376071`) can sign in until publishing completes. Business Verification for GARAGE CHERRIES LLC cleared 2026-07-09 (same day it was submitted), so the blocker is gone, but the actual Publish step on the Meta dashboard's Publish page (same as how Google was published to production) has not been done yet — this is a distinct, still-open action separate from the Facebook Page auto-posting work below, which used the same cleared verification for a different app (added 2026-07-09, verification/publish status corrected 2026-07-13)
