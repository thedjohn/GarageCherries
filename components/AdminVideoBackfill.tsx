...
export default function AdminVideoBackfill() {
  ...
  const trigger = async () => {
    setResult('loading');
    try {
      const res = await fetch('/api/admin/backfill-video-reels', {
      ...
      <div className="bg-white border border-zinc-100 rounded-2xl p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-bold text-zinc-900">Catch Up Missing Platform Videos</h2>
              <p className="text-sm text-zinc-500 mt-0.5">
                Finds listings missing a video on Facebook, Instagram, YouTube, or TikTok and queues a video render for each. Skips platforms a listing already has — safe to run anytime, including repeatedly.
              </p>
            </div>
          </div>
          <button
        ...
        {result && result !== 'loading' && (
          <div className={`mt-4 rounded-xl px-4 py-3 text-sm ${result.ok ? 'bg-green-50 border border-green-200 text-green-800' : 'bg-red-50 border border-red-200 text-red-800'}`}>
            {result.ok ? (
              <span><strong>Queued {result.triggered ?? 0}</strong> listing{result.triggered === 1 ? '' : 's'} for video generation.</span>
            ) : (
