...
const PAGE_SIZE = 3; // 1 row at the 3-column (lg) breakpoint -- these are supplementary to the article content, not the primary browse surface

function slugify(s: string) {
  return s.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
}

export default async function DecadePage({ content, page = 1 }: { content: DecadeContent; page?: number }) {
  ...
  const { data: dbRows, count } = await supabase
    ...
    .order('id', { ascending: true }) // tiebreaker -- see app/listings/page.tsx for why
