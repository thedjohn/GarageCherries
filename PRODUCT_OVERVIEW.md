...
- **Impressions vs. views** (scoped 2026-08-08, not started) — "impressions" means a listing appeared in a search/browse grid; "views" means the detail page was actually opened. We only capture the latter today — `listing_views` (via `components/ViewTracker.tsx`) fires exclusively on the listing detail page, never when a `CarCard` renders in `/listings` or homepage grids. Building this means adding view-tracking to `CarCard` itself (likely batched/throttled — a search results page can render 20+ cards at once) and a new column or table to distinguish impression counts from view counts.
- **Phone/website click tracking** (scoped 2026-08-08, not started) — the "Call Dealer" and "Visit website" links (`app/listings/[...segments]/page.tsx`) are plain `<a href>` tags today with no `onClick` handler and no tracking call of any kind — confirmed directly in the code, not assumed. GA4's Enhanced Measurement does generically catch these as "outbound click" events in GA4's own platform (confirmed earlier this session), but nothing writes them to our own Supabase tables, so there's no way to attribute a click to a specific dealer/listing in our own system today. The `ad_events` table + `/api/ads/serve` + `/api/ads/track` pipeline (already built for the advertiser dashboard) is a proven, working pattern for exactly this kind of per-entity click tracking and could likely be adapted rather than built from scratch.
- **AI-generated pricing insight** ("your car has X views but no inquiries, similar cars priced Y% lower are converting") — this is a specific, natural-language version of the "suggested price adjustments" bullet already above. The underlying comparable-listing data (make/model/year/price/mileage across the whole platform) already exists in the `listings` table and could power a basic statistical version (e.g., "average price for similar listings: $X") without AI at all. The natural-language "insight" framing specifically needs AI text generation, which requires re-enabling AI on the site — all AI features (including the prior Price Assessment feature) were removed 2026-07-01 and are currently deferred; see `Legal-Review-Checklist.md` item #14.
