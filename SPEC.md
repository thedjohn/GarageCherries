...
> Generated 2026-07-02 from a full read of every route, page, and library file. Last updated 2026-07-18 (see entry immediately below for the full list of this session's fixes and features).
...
| Sitemap expansion | **Complete (2026-07-09, updated 2026-07-17)** | `app/sitemap.ts` covers: all static pages (incl. `/events`, `/dealer/apply`, `/advertiser/signup`, `/privacy`, `/terms`, `/contact`, `/feedback`), listings (individual + make + make/model combos, now filtered to `status = 'approved'` only — was unfiltered, so rejected listings produced dead 404 links and pending ones surfaced unreviewed content), dealer pages, encyclopedia index + make + model pages (99 models across 20 makes, incl. the `/cars/srt` brand-overview page), advertiser directory + individual profiles, guide articles (6). Revalidates every 5 minutes (was hourly — found live that 6 of 8 real listings were unreachable via the sitemap right after creation because the cached version hadn't regenerated yet). |
