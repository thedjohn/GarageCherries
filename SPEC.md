...
> Generated 2026-07-02 from a full read of every route, page, and library file. Last updated 2026-09-07, one more time (Added a one-line explanatory subtitle under "Questions & Comments" on the listing page -- Derek, viewing the live site, noted it didn't explain what it was or how it differs from the "Message Seller" button on the same page. `components/ListingComments.tsx` only; no logic changed. Commit `680d6ab`. See IMPLEMENTATION_STATUS.md's latest 2026-09-07 entry for full detail.)
> Prior update 2026-09-07, yet later still (Live-tested the public listing-comments feature end to end through the real UI with throwaway accounts -- post a question, seller reply (with immediate email confirmed in server logs), report, and delete (confirmed via direct DB check to actually cascade-delete server-side). Two follow-up questions from Derek surfaced real answers: comments survive both feed-sync updates and manual edits (confirmed the sync code does a plain `UPDATE` on the existing row, not delete-and-reinsert; the one exception is a feed listing whose photos vanish entirely, which deletes the row outright and cascades); and admins could not delete a public comment at all before this -- fixed by adding a `requireAdmin()` check to `app/api/listings/[id]/comments/[commentId]/route.ts`, restricted to moderator role and above (not "support") per Derek's explicit call, plus a matching Delete button in the admin Reported Comments section so the permission is actually usable. Commit `ff9e832`. See IMPLEMENTATION_STATUS.md's latest 2026-09-07 entry for full detail.)
> Prior update 2026-09-07, later still (Reviewed all 3 error categories on Bing Webmaster Tools' Recommendations page, not just "Title too long." The other two: a Moderate "lacks inbound links from high-quality domains" item is backlink/outreach work, not a code fix, already tracked separately; a Low "img tag does not have an ALT attribute defined" item (flagged on the 1966 Plymouth Barracuda listing) was real — `components/ImageGallery.tsx`'s invisible preload image (prefetches the adjacent photo for instant Prev/Next) had `alt=""`, which Bing's scanner flags regardless of decorative intent. Gave it the same descriptive alt text already used for the visible/thumbnail images in the same file -- a shared component every listing page uses, so it resolves the gap site-wide. Verified live via direct DOM inspection. Commit `35f2f64`. See IMPLEMENTATION_STATUS.md's latest 2026-09-07 entry for full detail.)
> Prior update 2026-09-07, later same day (Fixed Car Guide page `<title>` tags flagged by Bing Webmaster Tools as "Title too long" (>70 chars). The fixed suffix `Make Model (Years) — Specs, History & Buyer's Guide` was too long for any moderately long make/model combo — checked all 148 encyclopedia entries (not just Bing's 6 flagged examples) and found 29 exceeded the limit, worst case 85 characters. Shortened `app/cars/[make]/[model]/page.tsx`'s `<title>` to `Make Model Buyer's Guide`, matching the `openGraph.title` format already used two lines below it in the same function; years dropped from the `<title>` tag only, still shown on-page as a badge next to the H1. Verified programmatically across all 148 entries (worst case now 66 characters), plus live in the dev server. Commit `7986505`. See IMPLEMENTATION_STATUS.md's later 2026-09-07 entry for full detail.)
> Prior update 2026-09-07 (Public Q&A comments on vehicle listings, a new feature alongside the existing private messaging system, not a replacement. New `listing_comments` table (Q&A style: any logged-in buyer can post a top-level question, only the seller/team can reply, enforced both in `app/api/listings/[id]/comments/route.ts` via `isAuthorizedForSeller()` and via a matching RLS insert policy). New top-level questions are batched into an hourly digest email per listing (`app/api/cron/listing-comments-digest/route.ts`, same cadence as the existing `dealer-feed-sync` cron) rather than emailed per-comment, avoiding a spam flood on a popular listing; a seller's reply still emails the asker immediately since that has no burst risk. Reported comments surface in the existing admin Reported tab alongside reported private messages. Public display name is client-supplied and required (mirrors `dealer_reviews.reviewer_name`, never derived from account email, unlike private messaging's `sender_name`). One real schema correction made before shipping: `listings.id` is `text`, not `uuid` (caught by Supabase's own foreign-key-type-mismatch error on the first migration attempt). Commit `6eefa92`. See IMPLEMENTATION_STATUS.md's 2026-09-07 entry for full detail.)
> Prior update 2026-09-06 (Dealer team access (parent/child accounts) built end-to-end for Beverly Hills Car Club's Joe Harwick — new `dealer_members` join table (`dealer_id`/`user_id`, unique constraint, RLS additive-only) plus two shared helpers in a new `lib/dealerAuth.ts` (`resolveDealerId()` for dealer-only routes, `isAuthorizedForSeller()` for routes shared with private sellers, checking the pre-existing direct-match condition first so private sellers and existing dealers are unaffected); new `GET/POST/DELETE /api/dealer/team` route reuses the existing dealer-application invite-email pattern for both a genuinely new invitee and an already-existing Supabase Auth user; new parent-only Team tab on the dealer dashboard. Live QA against a throwaway dealer surfaced and fixed four related bugs: the dealer dashboard's Inquiries tab gained the ability to reply (wired to the existing `MessengerWidget`/`useMessenger()` system rather than new UI); `components/Header.tsx` and `app/account/page.tsx` now recognize team members via `dealer_members` the same way the dashboard itself does, fixing a team member having no way back to the dashboard and no redirect off the buyer-account page; `components/MessengerWidget.tsx` now aligns/colors messages by which side of the conversation they're actually on (the real `buyer_id`) rather than "did the current viewer send this," since a second person can now send seller-side messages. No existing route's public contract changed — every touched dealer-only or shared-with-private-sellers check preserves its exact prior behavior for every existing account. Commit `e439bdc`. See IMPLEMENTATION_STATUS.md's 2026-09-06 entry for full detail.)
> Prior update 2026-08-24 (Seventy-third, a long RK Motors dealer-support thread. A dealer's password-reset links traced to two real causes — Supabase Auth's slow default mailer (fixed via custom SMTP through Resend) and corporate email link-scanning burning the one-time token before the dealer could click it (worked around via a direct admin-API password reset) — which led to building a real admin-panel "Reset Password" action (`PATCH /api/admin/users`, new `reset-password` action) since none existed before. Commit `82ab7dc`. Separately, answering the dealer's question about lead visibility surfaced that dealer website/phone click-throughs were GA4-tracked only and invisible to dealers, and that GA4 itself couldn't even break them down by dealer since `dealer_id` (plus the app's own UTM attribution parameters) had never been registered as GA4 custom dimensions — registered live via the GA4 Admin UI. Built first-party click tracking instead of depending on the GA4 API: new `dealer_link_clicks` table (mirrors `listing_views`), new `POST /api/dealer/track-click` route, `TrackedLink.tsx` now logs both to GA4 and this table. Wired into the listing detail page, then extended to the dealer profile page and the `/dealers` directory grid after finding both used separate, untracked link components (`listing_id` relaxed to nullable via a second migration for these listing-less clicks). Dealer dashboard and the monthly dealer report email both gained a new click-through metric. Commits `c59b083`, `92fa4fc`. All verified live in production via direct API calls and database queries, not just tests. See IMPLEMENTATION_STATUS.md item #90 for full detail.)
> Prior update 2026-08-23 (Three threads. First, the same 1000-row Supabase/PostgREST select-cap bug pattern turned up a third time, this time on the `events` query in `app/sitemap.ts` — never paginated before because events had never crossed 1000 rows, until the second thread below pushed the real count past 13,000 and the sitemap's event pages silently truncated. Fixed with the existing `fetchAllRows()` helper; verified live the paginated query returns the true count (13,437 at fix time) while a plain unpaginated select still caps at 1000. Commit `f82d38f`. See IMPLEMENTATION_STATUS.md item #88 for full detail. Second, the events-insertion thread (see item #83 and earlier) was completely re-architected and pushed through full-depth coverage of every remaining state, replacing the earlier ChatGPT-relayed-list method with a direct scraper against motorevents.net's own per-state pages plus a shared classifier that excludes competitive motorsport content, private club meetings, non-automotive content, and out-of-state listings — including several confirmed one-off mislabeling bugs on MotorEvents' own site (Lime Rock Park tagged NY instead of CT, the Great Marques Concours d'Elegance tagged ME instead of NY, Charles Town tagged VA instead of WV, a Ludlow MA brewery tagged CT), each independently verified before exclusion. Small states got full manual review; large states (Florida's 4,907 raw listings and up) used the automated pipeline without individual per-event research, a scale-driven tradeoff Derek explicitly chose. All 50 states now have real coverage; total events grew from roughly 13,400 to 28,802. See IMPLEMENTATION_STATUS.md item #88 for full detail. Third, replaced the events "near me" filter's ZIP→state and free-text-city approximations with real distance-based search, after Derek pushed back that the shipped filter combo didn't feel like genuine "near me" search. Added `zipcodes-us` (a static, zero-dependency US ZIP/city coordinate dataset — this project's first data-utility npm package) and a new shared `lib/geo.ts` (extracted `STATE_CENTROIDS`/`haversineMiles()` out of `app/api/ads/serve/route.ts`, plus new `resolveZipCoords()`, `resolveEventCoords()`, `boundingBox()`). Added nullable `lat`/`lng` columns to `events`, wired both event-write paths to resolve coordinates at write time, backfilled all 28,802 existing events (26,219 matched a real city, 2,583 fell back to their state centroid, 0 failures), and rebuilt `EventFilters.tsx` so ZIP is a real URL param on every events page. Both `/events` and `/events/state/[state]` now do a DB bounding-box pre-filter plus exact JS-side haversine distance sort/paginate when a ZIP resolves (50-mile default radius), with the no-ZIP path's existing `.range()` pagination unchanged. Verified live end-to-end; full suite (1,275 tests) and coverage gate passing. See IMPLEMENTATION_STATUS.md item #89 for full detail.)
> Prior update 2026-08-20 (Two threads. First, completing Track 1 (the affiliate-referral track scoped 2026-08-07) — pure partner-outreach/business-development work, no code. Montway's Impact.com Publisher application was taken all the way through submission: created and categorized the account, connected all four channels the video pipeline already posts to (Website/Instagram/Facebook/TikTok/YouTube — one real hiccup, deselecting TikTok's "Creator Marketplace orders" scope on a least-privilege instinct broke the connection since Impact.com's integration requires it despite the UI allowing it to be toggled off; reconnected with full scope), and replaced the platform's AI-generated profile description/tags (which had scraped current live inventory model names rather than describing the business) with an evergreen version. Application status is now "In Review." Montway's own "Template Terms," read directly rather than assumed, show a literal $0.00-per-booking / 0%-of-sale default payout — flagged rather than accepted, with a direct email sent to `info@montway.com` (the only public contact address on their site) asking them to confirm the real rate, since the application flow itself never surfaced a way to ask inline. Also read and summarized Impact.com's own Master Program Agreement + Partner User Agreement (extracted via `pdftotext` after WebFetch's built-in parser failed on the binary PDF) — standard boilerplate, no exclusivity requirement, but confirmed Derek's payout depends on Montway funding their Impact.com account first, a real counterparty-risk detail beyond the rate question alone. Lemon Squad (real live `partner@wrench.com` contact re-verified after an earlier-session 503 outage resolved), Woodside Credit (no public email exists anywhere on their site — confirmed, not assumed; used their real 3-step "Dealer Resources" web form instead), and J.J. Best Banc (their "Dealer CONNECT" application form was recognized as built for a physical single-location dealership rather than an online marketplace, based on its address/vehicle-photo fields — redirected to a direct email to their real `cs@jjbest.com` contact instead of submitting a mismatched application) all had outreach drafted and sent by Derek. Net: all five Track 1 partners identified 2026-08-07 (Hagerty, Montway, Lemon Squad, Woodside Credit, J.J. Best Banc) now have real outreach sent or an application in progress; nothing left to build until a partner responds with a real tracking ID. See IMPLEMENTATION_STATUS.md item #18 for full detail. Second, the Independent Inspection Reports feature (the other genuine gap confirmed in 2026-08-07's ChatGPT-doc fact-check) was built: new `listing_inspections` table (see §3) plus a new `inspection-reports` Storage bucket, a dealer-scoped `GET/POST/DELETE /api/dealer/inspection` route, dealer-side upload UI mounted in the Edit Vehicle modal, and a buyer-facing summary card on the listing detail page with a disclaimer mirrored onto `/terms`. Live before/after verification (temporary test data inserted via a service-role script, viewed in the dev server, then reverted) caught two real bugs before they shipped: a UTC/local-timezone date-display bug (same class already fixed 2026-07-23) and a JSX whitespace-swallowing bug around an interpolated value. See IMPLEMENTATION_STATUS.md item #19 for full detail. Third, a post-sale review nudge was added to the existing "a car you watched just sold" email (new shared `lib/notifyCarSold.ts`, replacing the inline copy previously only in `app/api/cars/sold/route.ts`) — while scoping it, found and fixed a real pre-existing gap: the automated feed-sync mark-sold path (`app/api/cron/dealer-feed-sync/route.ts`, the more common way a feed-managed dealer's car actually goes sold) never sent this notification to anyone, only the manual "Mark as Sold" dashboard button did. Both paths now call the same shared function. `app/dealers/[slug]/page.tsx` gained an `id="reviews"` anchor for the email's review link to target. See IMPLEMENTATION_STATUS.md item #20 for full detail. Fourth, a homepage stats bar (`app/page.tsx`) shows real Active Listings/Dealers/Cars Sold counts between the hero and "Browse by Body Style" sections, replacing the previous vague "Growing daily" copy — purely additive, no existing homepage query or section touched. See IMPLEMENTATION_STATUS.md item #21 for full detail. Fifth, DMCA Safe Harbor registration was completed end-to-end: real registration with the U.S. Copyright Office (DMCA-1077785, Active), plus the required on-site publication — new `app/dmca/page.tsx` (takedown notice requirements, counter-notification process, repeat infringer policy), a footer link, and a new section on `/terms` linking to it. Closes out `Legal-Review-Checklist.md` item #1. See IMPLEMENTATION_STATUS.md item #22 for full detail. Sixth, FTC affiliate/referral disclosure infrastructure was built ahead of any partner link going live: new `app/affiliate-disclosure/page.tsx`, a new Terms of Service section, a footer link, and a reusable `components/AffiliateDisclosure.tsx` (not yet rendered anywhere — no real affiliate link exists yet to attach it to). See IMPLEMENTATION_STATUS.md item #23 for full detail. Seventh, no code changed: legal checklist item #3 (placeholder text) was verified against the live Terms/Privacy Policy and the real Missouri Secretary of State business filing — placeholders were already stale, registered agent address confirmed correct, and a minor legal-name spacing difference ("Garage Cherries LLC" vs. the Site's "GarageCherries LLC") was found and left as-is per Derek's decision. See IMPLEMENTATION_STATUS.md item #24 for full detail. Eighth, legal checklist item #4 (Data Breach Notification Policy) was built: a new "Data Breach Notification" section on `/privacy` (30-day notification commitment, breach definition) and a new internal `Incident-Response-Plan.md`. Closes out all four Priority 1 items on `Legal-Review-Checklist.md` for the first time this session. See IMPLEMENTATION_STATUS.md item #25 for full detail. Ninth, `components/FinancingCalculator.tsx`'s disclaimer was strengthened (estimate only, not an offer of credit, GarageCherries is not a lender) as a risk-reduction step on legal checklist item #7 — the underlying TILA/Regulation Z question about displaying APR alongside payment/term amounts remains open for attorney review. See IMPLEMENTATION_STATUS.md item #26 for full detail. Tenth, CAN-SPAM gaps closed: a physical mailing address was added once to the shared `emailWrap()` function (`lib/emailBranding.ts`) so it applies to every email automatically, and a missing unsubscribe link on the new car-sold notification (`lib/notifyCarSold.ts`) was fixed with a new `app/unsubscribe/car-sold/page.tsx` and a `car_sold_opt_out` user-metadata flag, mirroring the existing price-drops unsubscribe pattern exactly. See IMPLEMENTATION_STATUS.md item #27 for full detail. Eleventh, two roadmap items (impressions/click tracking, AI-generated pricing insight) were added to `PRODUCT_OVERVIEW.md` after fact-checking a ChatGPT-generated dealer-analytics wishlist against the live site; no code changed. See IMPLEMENTATION_STATUS.md item #28 for full detail. Twelfth, 38 car show events were inserted from four ChatGPT-sourced lists via one-off service-role scripts, then independently re-verified against real primary sources rather than trusted as-is: 7 of 25 checked events had no verifiable evidence anywhere and were removed, 14 got real corrections (wrong ordinals, wrong times, one wrong date — Hot Rods in the Park's real date is Aug 30 2026, not Sep 5 as inserted — caught independently during the correction pass), and 3 more were removed for lacking any confirmed 2026 date (one of those, Passion Community Church, also turned out to be sourced to the wrong city and a prior year). Final count: 127 real, sourced events. No schema or code changes — data-only via scratch scripts, deleted after each use. See IMPLEMENTATION_STATUS.md item #29 for full detail. Thirteenth, the `/cars` Classic Car Encyclopedia (`lib/encyclopedia.ts`) was audited against real inventory using the site's own Live Inventory matching rule (`app/cars/[make]/[model]/page.tsx`), catching a real bug: the Oldsmobile "4-4-2" entry's `model` field (`4-4-2`) never prefix-matched real listings (which store `442`), leaving 5 real listings invisible on their own encyclopedia page. Fixed via a new optional `matchModel?: string` field on `EncyclopediaEntry` that overrides `model` only for the inventory-match calculation (default: falls back to `model`, so all pre-existing entries are unaffected) — no URL/slug change. Also added 6 new encyclopedia entries for real, previously-uncovered inventory categories: Chevrolet C10, Chevrolet 3100, Chevrolet Suburban, Chevrolet 210, Ford F-100 (also using `matchModel` to bridge the `F-100`-display-vs-`F100`-stored naming gap), and Ford Model A — closing Live Inventory coverage for 26 previously-uncovered real listings. Purely additive to a static content file plus one guarded query-line change; no auth/login code touched. See IMPLEMENTATION_STATUS.md item #30 for full detail. Fourteenth, three more encyclopedia entries were added at Derek's request, following up on the two gaps flagged but left out of scope in the prior thread: Ford F-250 (`matchModel: 'F250'`, same technique as F-100), GMC 100 (the real GMC "New Design" half-ton, verified via web search before writing rather than assumed from memory — GMC's actual 1947–55 model numbers are 100/150/250, not Chevrolet's 3100), and GMC Sierra (covering Sierra's real history as a C/K trim package before becoming GMC's model name in 1988). Also surfaced, not corrected: a real inventory listing titled "GMC 3100" is almost certainly mislabeled, since GMC never used that number. See IMPLEMENTATION_STATUS.md item #31 for full detail. Fifteenth, closed the two remaining real coverage gaps by making `EncyclopediaEntry.matchModel` (`lib/encyclopedia.ts`) accept `string | string[]` instead of just `string` — backward compatible, every pre-existing entry is unaffected. `app/cars/[make]/[model]/page.tsx`'s query now builds one `model.ilike.<prefix>%` clause per match word and combines them via `.or()`, chained alongside the existing `expires_at` `.or()` (PostgREST ANDs separate `.or()` calls together). Ford F-250 (`matchModel: ['F250', 'F-250']`) and GMC Sierra (`matchModel: ['Sierra', 'C1500']`) both updated to use it, picking up one previously-uncovered real listing each without writing new near-duplicate content or touching real dealer listing data. See IMPLEMENTATION_STATUS.md item #32 for full detail. Sixteenth, added a Ford F-150 encyclopedia entry (1975–96) after Derek asked why one didn't exist — no F-150 currently exists in real inventory, so the earlier inventory-driven audit method couldn't have surfaced this gap, but it's justified independent of current stock given F-150's status as Ford's flagship half-ton and America's best-selling vehicle for decades. Set `matchModel: ['F150', 'F-150']` from the start (learned from the prior thread's F-250 fix) so either real-listing spelling will match once one exists. See IMPLEMENTATION_STATUS.md item #33 for full detail. Seventeenth, re-ran the inventory-vs-encyclopedia audit against the now-146-entry encyclopedia and closed three more real gaps: extended Chevrolet C10's `matchModel` to also catch `C/K`, `K5`, `K-10`, `Apache`, `Silverado` (widening the entry's stated years from 1960–87 to 1958–87 to accurately cover Apache) and Chevrolet 3100's to also catch `3600`, rather than writing new near-duplicate content; and added 6 new entries for two brand-level gaps with zero prior coverage — Dodge Power Wagon, Dodge Ramcharger, Dodge Ram (1981–2001, matches both "D-150" and "1500 Quad Cab" listings via a shared "Ram" prefix, no array needed), Jeep CJ-7 (`matchModel: ['CJ-7', 'CJ7']`), Jeep Wrangler, and Jeep Commando. See IMPLEMENTATION_STATUS.md item #34 for full detail. Eighteenth, no code changed except one direct data correction: re-ran the audit a third time and found no more real content clusters (deliberately declined to widen Ford Model A's match to catch its remaining generic-titled listings — "Coupe"/"Sedan"/"Roadster" are too generic to safely prefix-match). Surfaced a Mercedes/Mercedes-Benz make-string mismatch affecting 3 listings, plus 3 genuinely uncovered Mercedes generations (Pagoda SL, R129 SL500, CLK/GL-Class) — recommended a code-side `matchMake` fix (mirroring `matchModel`) rather than renaming real listings, not yet built. Separately, one listing's `make: 'Classic'` was corrected to `'Glassic'` (the real manufacturer, confirmed via the listing's own description and Derek's sign-off) — id `61fd40b8-c8a8-4a58-8f3e-d7a1c7aaf8be`, fixed via direct script, no encyclopedia entry needed for a single listing. See IMPLEMENTATION_STATUS.md item #35 for full detail. Nineteenth, built the `matchMake` fix recommended in #35: `EncyclopediaEntry` gained `matchMake?: string | string[]` (mirrors `matchModel`, only affects Live Inventory matching, never the entry's URL/slug), and `app/cars/[make]/[model]/page.tsx`'s query changed from `.eq('make', entry.make)` to `.in('make', matchMakes)` — a structural change affecting every encyclopedia page's make-matching, regression-tested against entries with no `matchMake` set to confirm the fallback behaves identically to the old exact match. All 4 existing Mercedes entries now set `matchMake: ['Mercedes', 'Mercedes-Benz']`. Added 4 new entries closing the real gaps found in #35: Mercedes 250SL (W113 Pagoda, 1963–71), SL500 (R129, 1989–2001), CLK (1997–2009), and GL Class (2006–16, `matchModel: ['GL', 'AMG']` to catch two differently-worded real listings for the same SUV lineage — a deliberately broader match, documented as a known tradeoff). See IMPLEMENTATION_STATUS.md item #36 for full detail. Twentieth, a reported 404 traced to a real bug: the hardcoded `MAKES` array (`lib/types.ts`) that drives the site's make filter dropdown and the `/listings/[make]/[model]` browse route (a separate code path from the working listing-detail route) was missing 6 real manufacturers from live inventory. Added `Austin` and `Subaru` (no scope ambiguity); left out `Harley Davidson` (motorcycles, a product-scope decision) and `IR` (turned out to be a golf cart, not a car — investigated the real listing directly rather than guessing). Fixed the Mercedes/Mercedes-Benz split at its actual source rather than with a one-off DB correction, after confirming via Vaughns' and Survivor's real live feeds that both affected listings (plus the already-corrected Glassic listing) belong to actively feed-synced dealers whose sync unconditionally overwrites `make` with whatever the feed sends — new `MAKE_ALIASES`/`normalizeMake()` in `app/api/cron/dealer-feed-sync/route.ts`, applied once at parse time so every downstream use stays consistent. See IMPLEMENTATION_STATUS.md item #37 for full detail. Twenty-first, built the conditional "Classic"→"Glassic" alias after all — checking Survivor's own website (not just their feed) confirmed the mislabeling is systemic on their end, which ruled out a blanket alias but not a conditional one: `CONDITIONAL_MAKE_ALIASES` in `dealer-feed-sync/route.ts` only fires when the giveaway word "Glassic" also appears in `Sub-Model` or a newly-parsed `VDP URL` column, narrow enough to be safe. See IMPLEMENTATION_STATUS.md item #38 for full detail. Twenty-second, removed the `MAKES`-list dependency from `/listings/[make]` and `/listings/[make]/[model]` (`app/listings/[...segments]/page.tsx`) entirely, after finding this exact bug class had already been hit and worked around once before (a `sitemap.ts` comment documents excluding Mercedes-Benz from the sitemap rather than fixing the route). Both routes now resolve the make segment against real live listings data, the same way the model segment already did, rather than gating on the static array — `MAKES` itself is untouched and still used for the filter dropdown/footer links, just no longer used to block access to a real listing's page. See IMPLEMENTATION_STATUS.md item #39 for full detail. Twenty-third, 12 more car show events (Ohio) added via a one-off data script, supplied this time as a pre-structured JSON spec with explicit field mappings and a name+date+city+state dedup key rather than a ChatGPT conversation — checked the real `events` schema/type enum first, ran the same duplicate check as prior batches (0 collisions, all 12 net-new), and did not independently re-verify against primary sources since this request didn't ask for that step. Total events: 127 → 139. No code changed. See IMPLEMENTATION_STATUS.md item #40 for full detail. Twenty-fourth, a real 100x listing price error was fixed after Derek compared it directly against the dealer's own site — Atomic Motors' 1985 Chrysler LeBaron corrected from $3,798,000 to $37,980; safe to fix directly since this dealer's listings are manually managed, not feed-synced. See IMPLEMENTATION_STATUS.md item #41 for full detail. Twenty-fifth, 8 more Ohio car show events added via the same pre-structured-spec process as the prior batch (0 duplicates). Total events: 139 → 147. See IMPLEMENTATION_STATUS.md item #42 for full detail. Twenty-sixth, a new `/cars/muscle-cars` page (`app/cars/muscle-cars/page.tsx`, content in `lib/muscleCars.ts`) built after an SEO audit found no page on the site actually targeted "muscle cars for sale" — "muscle car" isn't a `body_style` value, so it fell outside the existing `BodyStylePage`/`BODY_STYLES_CONTENT` system entirely. Built standalone rather than extending that shared component: fetches approved/unsold listings for a curated 10-make list via `.in('make', ...)`, then filters to ~30 real model-name prefixes and paginates in memory. Wired in via a second `/cars` Featured Guide card (above the existing untouched SRT one), a new Footer "Muscle Cars" link, and a hand-added `/sitemap.xml` entry at priority 0.8. 83 real listings matched live. See IMPLEMENTATION_STATUS.md item #43 for full detail. Twenty-seventh, real per-state event pages (`app/events/state/[state]/page.tsx`) built after finding `/events?state=X` had no unique title *and* a canonical hardcoded to the generic `/events` URL regardless of query string. New `lib/usStates.ts` (2-letter code ↔ full name ↔ URL slug). Since `app/events/[slug]/page.tsx` already occupies the `/events/[x]` path shape for individual events, the new pages live at the non-colliding `/events/state/[state]` instead — reusing `EventCard`/formatters newly exported (not duplicated) from `app/events/page.tsx`. The existing `?state=X` query view is unchanged for users; its canonical now points at the new page when a state filter is active. New "Browse by State" section on `/events` with real internal links, sorted by real event count. New sitemap entries for all 16 states with real events. See IMPLEMENTATION_STATUS.md item #44 for full detail. Twenty-eighth, no code changed: a wrong-priced social video (the 2026-08-08 Atomic Motors LeBaron correction, already baked into a posted Facebook/Instagram Reel before the fix) was corrected by clearing `reel_posted_at`/`instagram_posted_at` and triggering a direct single-listing re-render via the VPS, after Derek manually deleted the old wrong-priced posts — a real near-miss along the way (`fb_posted_at`, a separate field gating an unrelated cron, was mistakenly cleared and immediately reverted before it could queue an unintended post). See IMPLEMENTATION_STATUS.md item #45 for full detail. Twenty-ninth, 8 more car show events (North Carolina) added via the same process as prior batches, this one adding a fuzzy name-matching dedup check (word-overlap ratio) alongside the existing exact match, per the request's explicit spec. 0 duplicates, all 8 net-new. Total events: 147 → 155. See IMPLEMENTATION_STATUS.md item #46 for full detail. Thirtieth, 8 more North Carolina events added via the same process, this batch's spec adding an explicit rule that different dates of a recurring event are not duplicates of each other — already correctly handled by the existing per-date fuzzy match with no code changes needed. 0 duplicates, all 8 net-new. Total events: 155 → 163 (NC: 9 → 17). See IMPLEMENTATION_STATUS.md item #47 for full detail. Thirty-first, first South Carolina events batch (13 events, new 17th state) — caught a real false-positive bug in the fuzzy-matching script itself: 2 genuinely distinct events (different cities/organizations) were wrongly flagged as duplicates because the stopword list didn't exclude generic car-show vocabulary ("car," "show," "church," "and"), letting boilerplate words alone push the overlap score over the 60% threshold. Fixed the stopword list, re-checked, confirmed both were net-new, inserted them. Total events: 163 → 176. No app code changed — the matching logic lives only in the one-off insertion scripts, not the deployed app. See IMPLEMENTATION_STATUS.md item #48 for full detail. Thirty-second, first Georgia events batch (12 events, new 18th state) — added a same-date-and-city review check alongside the existing name-similarity fuzzy match, since the same real event can be entered under a completely different name (its own name vs. a beneficiary organization's name) that word-overlap alone can't catch. Nothing flagged; all 12 net-new. Total events: 176 → 188. See IMPLEMENTATION_STATUS.md item #49 for full detail. Thirty-third, first Florida events batch (10 events, new 19th state) — implemented a real venue-comparison check (extracting street-address text from event descriptions) per this request's spec, auto-skipping only when both city and venue text overlap. All 10 net-new. Total events: 188 → 198. See IMPLEMENTATION_STATUS.md item #50 for full detail. Thirty-fourth, first Alabama events batch (11 events, new 20th state) — reused the same dedup pipeline unchanged; confirmed the request's illustrative duplicate examples resolve correctly under the existing matching logic. All 11 net-new. Total events: 198 → 209. See IMPLEMENTATION_STATUS.md item #51 for full detail. Thirty-fifth, first Alaska events batch (5 events, new 21st state), same pipeline. Total events: 209 → 214. See IMPLEMENTATION_STATUS.md item #52 for full detail. Thirty-sixth, first Arizona events batch (19 events, new 22nd state, the largest single batch of this thread) — same pipeline, explicitly checked for internal same-date+city collisions given several repeated cities, nothing flagged. All 19 net-new. Total events: 214 → 233. See IMPLEMENTATION_STATUS.md item #53 for full detail. Thirty-seventh, first Arkansas events batch (11 events, new 23rd state), same pipeline. Total events: 233 → 244. See IMPLEMENTATION_STATUS.md item #54 for full detail. Thirty-eighth, first New England batch (13 events across Massachusetts, New Hampshire, and Connecticut — 3 new states in one request), same pipeline, confirmed the existing per-state-and-date scoping handles multiple states in one batch correctly with no changes needed. All 13 net-new. Total events: 244 → 257, 26 states covered. See IMPLEMENTATION_STATUS.md item #55 for full detail. Thirty-ninth, first Delaware events batch (20 events across Delaware and Pennsylvania — DE a new 27th state, PA already covered), same pipeline — the venue check flagged one real same-city/same-date collision (two new Middletown, DE events sharing 2026-08-15 with a pre-existing "Middletown Car, Truck & Tractor Show"), checked and confirmed genuinely distinct, not skipped. All 20 net-new. Total events: 257 → 277 (DE: 0 → 17, PA: 4 → 7), 27 states covered. See IMPLEMENTATION_STATUS.md item #56 for full detail. Fortieth, first Hawaii events batch (5 events, a new 28th state), same pipeline, all 5 net-new with nothing flagged. Total events: 277 → 282, 28 states covered. See IMPLEMENTATION_STATUS.md item #57 for full detail. Forty-first, first Louisiana events batch (19 events, a new 29th state), same pipeline — three cities each appeared twice on different dates, correctly kept separate. All 19 net-new. Total events: 282 → 301, 29 states covered. See IMPLEMENTATION_STATUS.md item #58 for full detail. Forty-second, first Maine events batch (7 events, a new 30th state), same pipeline, all 7 net-new with nothing flagged. Total events: 301 → 308, 30 states covered. See IMPLEMENTATION_STATUS.md item #59 for full detail. Forty-third, first DMV-area batch (15 events: 1 Maryland, a new 31st state, and 14 Virginia) — surfaced a real gap in the dedup script: 2 duplicates weren't caught by the location-comparing checks because the pre-existing records store a full venue address in `location` rather than just the city, but both were caught by the database's own slug-uniqueness constraint before any bad data was written. A 3rd duplicate was caught normally. 12 of 15 net-new. Total events: 308 → 320 (VA: 63 → 74, MD: 0 → 1), 31 states covered. See IMPLEMENTATION_STATUS.md item #60 for full detail. Forty-fourth, a 14-event batch across six states (Massachusetts/Iowa already covered; Mississippi, Montana, Nebraska, New Jersey new — 32nd–35th states), applying the prior thread's flagged fix — the fuzzy-match check now flags same-date/same-state name matches with non-overlapping city text for manual review instead of silently passing them through. Nothing flagged, all 14 net-new. Total events: 320 → 334, 35 states covered. See IMPLEMENTATION_STATUS.md item #61 for full detail. Forty-fifth, first New Mexico and first New York batch (10 events, both new — 36th and 37th states), same pipeline, nothing flagged, all 10 net-new. Total events: 334 → 344, 37 states covered. See IMPLEMENTATION_STATUS.md item #62 for full detail. Forty-sixth, a second New York batch (8 events, all Long Island area), same pipeline, no overlap with the prior thread's Western New York events, all 8 net-new. Total events: 344 → 352 (NY: 5 → 13). See IMPLEMENTATION_STATUS.md item #63 for full detail. Forty-seventh, first North Dakota events batch (15 events, a new 38th state), same pipeline — two recurring series each appeared twice on different dates, correctly kept separate. All 15 net-new. Total events: 352 → 367, 38 states covered. See IMPLEMENTATION_STATUS.md item #64 for full detail. Forty-eighth, first Oklahoma events batch (21 events, largest single batch so far, a new 39th state), same pipeline — Oklahoma City appeared 5 times on different dates, nothing flagged. All 21 net-new. Total events: 367 → 388, 39 states covered. See IMPLEMENTATION_STATUS.md item #65 for full detail. Forty-ninth, a Pacific Northwest batch (15 events: Washington already covered, Oregon new — 40th state), same pipeline, all 15 net-new. Total events: 388 → 403 (WA: 1 → 12, OR: 0 → 4), 40 states covered. See IMPLEMENTATION_STATUS.md item #66 for full detail. Fiftieth, first Rhode Island events batch (10 events, a new 41st state), same pipeline — Newport and Warwick each appeared multiple times on different dates, correctly kept separate. All 10 net-new. Total events: 403 → 413, 41 states covered. See IMPLEMENTATION_STATUS.md item #67 for full detail. Fifty-first, first South Dakota events batch (14 events, a new 42nd state), same pipeline — Sturgis and a recurring market show each appeared multiple times on different dates, correctly kept separate. All 14 net-new. Total events: 413 → 427, 42 states covered. See IMPLEMENTATION_STATUS.md item #68 for full detail. Fifty-second, first Tennessee events batch (27 events, largest single batch so far, a new 43rd state), same pipeline — several cities recurred across different dates, nothing flagged. All 27 net-new. Total events: 427 → 454, 43 states covered. See IMPLEMENTATION_STATUS.md item #69 for full detail. Fifty-third, first Texas events batch (15 events, a new 44th state), same pipeline, all 15 net-new. Total events: 454 → 469, 44 states covered. See IMPLEMENTATION_STATUS.md item #70 for full detail. Fifty-fourth, first Utah events batch (14 events, a new 45th state), same pipeline — Salt Lake City and Midvale each appeared twice on different dates, correctly kept separate. All 14 net-new. Total events: 469 → 483, 45 states covered. See IMPLEMENTATION_STATUS.md item #71 for full detail. Fifty-fifth, first Vermont events batch (12 events, a new 46th state), same pipeline — two recurring series each appeared multiple times on different dates, correctly kept separate. All 12 net-new. Total events: 483 → 495, 46 states covered. See IMPLEMENTATION_STATUS.md item #72 for full detail. Fifty-sixth, first West Virginia events batch (25 events, a new 47th state), same pipeline — three recurring series each appeared twice on different dates, correctly kept separate. All 25 net-new. Total events: 495 → 520, 47 states covered. See IMPLEMENTATION_STATUS.md item #73 for full detail. Fifty-seventh, first Wisconsin events batch (25 events, a new 48th state), same pipeline — one same-city/same-date pre-existing event logged for visibility but confirmed genuinely unrelated, not skipped. All 25 net-new. Total events: 520 → 545, 48 states covered. See IMPLEMENTATION_STATUS.md item #74 for full detail. Fifty-eighth, first Wyoming events batch (8 events, a new 49th state — Idaho now the only state with zero events), same pipeline, all 8 net-new. Total events: 545 → 553, 49 states covered. See IMPLEMENTATION_STATUS.md item #75 for full detail. Fifty-ninth, first Idaho events batch (10 events) — completes real event coverage for all 50 states. Same pipeline, all 10 net-new. Total events: 553 → 563. See IMPLEMENTATION_STATUS.md item #76 for full detail. Sixtieth, fixed a real bug: the admin dashboard's pending-listing "View" link 404'd because the public listing page's anon Supabase client is blocked by RLS from seeing non-approved listings. Added an admin-only fallback in `app/listings/[...segments]/page.tsx` that retries with the service-role client only for a verified logged-in admin — anonymous visitors still get a real 404 for non-approved listings. See IMPLEMENTATION_STATUS.md item #77 for full detail. Sixty-first, a second Maryland events batch (33 events), same pipeline, all 33 net-new. Total events: 563 → 596 (MD: 1 → 34), 50 states covered. See IMPLEMENTATION_STATUS.md item #78 for full detail. Sixty-second, a second Minnesota events batch (19 events), same pipeline, all 19 net-new. Total events: 596 → 615 (MN: 1 → 20). See IMPLEMENTATION_STATUS.md item #79 for full detail. Sixty-third, a second Kansas events batch (36 events), same pipeline — caught and verified one real duplicate via fuzzy name matching before skipping it. 35 of 36 net-new. Total events: 615 → 650 (KS: 1 → 36). See IMPLEMENTATION_STATUS.md item #80 for full detail. Sixty-fourth, a second Indiana events batch (24 events), same pipeline, all 24 net-new. Total events: 650 → 674 (IN: 1 → 25). See IMPLEMENTATION_STATUS.md item #81 for full detail. Sixty-fifth, a second Kentucky events batch (14 events), same pipeline — Bowling Green recurred 5 times and a cruise-in series twice, all on different dates, correctly kept separate. All 14 net-new. Total events: 674 → 688 (KY: 1 → 15). See IMPLEMENTATION_STATUS.md item #82 for full detail. Sixty-sixth, a second Colorado events batch (19 events) — CO was the last remaining thin state, closing out real depth across all 50 states. Caught and verified one real duplicate (a national Goodguys event, same dates/city as an existing thinner record) before skipping it without overwriting. 18 of 19 net-new. Total events: 688 → 706 (CO: 1 → 19). See IMPLEMENTATION_STATUS.md item #83 for full detail. Sixty-seventh, an SEO strategy audit (technical/content/performance research plus external 2026-best-practices research) found the site's real bottleneck is discoverability/authority, not content depth — followed by Phase 1 (internal linking) implementation: footer links for `/reports`/`/cars/srt` added, body-style footer links repointed from thin filtered-search URLs to the actual long-form guide pages, listing detail pages now link to matching encyclopedia entries, event pages now link to state listings/dealers, and a guide's plain-text encyclopedia mention became a real link. 5 files changed. See IMPLEMENTATION_STATUS.md item #84 for full detail. Sixty-eighth, a silent-undercounting bug — Supabase/PostgREST's default 1000-row cap on any unbounded `.select()`, silently truncating results computed in JS rather than via `count()`/`.range()` — was found and fixed across six spots: the homepage Active Listings stat, Admin Overview's pending-queue count/oldest-pending-date, the Dealers directory's per-dealer active-listing badges, Admin Users' per-user listing/watchlist/conversation counts, `/cars/muscle-cars`, and `/reports` (Market Report). The last three now share a new `fetchAllRows()` helper (`lib/db.ts`) that pages any query past the cap in 1000-row batches, ordered by `id` (or a real display-order column plus `id` as a tie-breaker) for gap-free/duplicate-free paging; Admin Overview/Dealers instead moved to targeted `count()`/`.limit(1)` queries since they only ever needed a count or a single row, not the full fetched set. Verified against real data crossing the cap (a 1500-row test fixture for Admin Overview's pending count) and live in the authenticated admin UI — Beverly Hills Car Club's 533 active listings matched a direct database count. No schema or API-contract changes. See IMPLEMENTATION_STATUS.md item #85 for full detail. Sixty-ninth, SEO backlink-outreach prep (`~/.claude/plans/cryptic-strolling-cook.md` Phase 2/3) plus one real bug fix it surfaced. Phase 2 (structured data) was re-verified against the live codebase before implementing — all 4 planned items were already shipped from an earlier session, so no code changes were needed there. Phase 3 (backlink outreach) queried the real `events` table and produced 5 draft outreach emails targeting 8 events across 5 real organizations — business/outreach material, not yet sent, no schema or API-contract impact. While drafting those emails, found and fixed a real bug: the homepage's `Organization` JSON-LD schema (`app/page.tsx`) referenced a nonexistent `/logo.png`, a live 404 in the site's own structured data — repointed to the real asset, `cherry-logo.png`. Verified live (was 404, now 200). Commit `5a76ac6`. No schema or API-contract changes. See IMPLEMENTATION_STATUS.md item #86 for full detail. Seventieth, two more real bugs, no schema or API-contract changes. The same 1000-row select-cap bug from the prior thread was found in `app/sitemap.ts` (1086 approved listings vs. the 1000-row default cap silently dropped 82 real listings from `sitemap.xml`, caught by the daily `sitemap-health` cron) — fixed with the existing `fetchAllRows()` helper. Commit `9f34ff2`. Separately, a dealer-reported password-reset failure traced to a real, systemic config/copy mismatch: Supabase's Auth "Email OTP expiration" was set to 3600 seconds (1 hour) while every dealer setup/reset email's own copy has always promised "This link expires in 24 hours" (`app/api/admin/dealer-applications/route.ts`) — fixed by changing the Supabase dashboard setting to 86400 seconds to match the existing email copy, no code change. See IMPLEMENTATION_STATUS.md item #87 for full detail.)
> Prior update 2026-08-07 (Beverly Hills Car Club reached out about inventory syndication with an XML feed — the first non-CSV vendor encountered; investigated by fetching their real 544-vehicle feed directly and converting it with a one-off local script (no new dependency, nothing committed) rather than assuming, which confirmed clean data (100% USD, no duplicate IDs, no malformed prices) but a real gap (no city/state, country only) and a bug in the diagnostic script itself (641 unescaped `&amp;` entities never decoded). Decided against building native XML parsing for now — one confirmed prospect doesn't justify a new dependency and touching `syncDealerFeed()`, the function every working dealer's daily sync depends on — asked the vendor for a CSV export instead; also fixed a previously-undocumented bug where `dealer-feed-sync/route.ts`'s update path was overwriting a manually-corrected `make`/`model` with a blank value from the feed every sync (only writes those columns now when the feed actually sends something, commit `766ea58`). Separately, Google's YouTube API Compliance Team rejected the earlier quota-review screencast evidence and asked for a screencast of the actual upload process plus one showing an embedded video playing on garagecherries.com itself — the second ask surfaced that the site never actually embedded/played YouTube videos anywhere, only cross-posted to YouTube as external content, so built a real on-site embed feature: `postListingReelToYouTube()` now returns the real video ID instead of a boolean, a new `listings.youtube_video_id` column captures it, and a new click-to-play `components/YouTubeEmbed.tsx` (privacy-enhanced `youtube-nocookie.com` domain, no autoplay) renders on the listing detail page when present. Confirmed working end-to-end against real production data the same day: triggered a single-listing render via a one-off script against the VPS, traced a `youtube_posted_at`/`youtube_video_id` non-update on that specific listing to a concurrent hourly backfill batch exhausting the daily public-upload quota (not a bug — a full local replication of the upload chain succeeded cleanly), and verified the embed live on two other real listings from that same batch that did get a `youtube_video_id`. Then backfilled `youtube_video_id` for 121 of 122 pre-existing listings that had a real YouTube upload from before the column existed — re-authorized the YouTube OAuth connection with an added read-only scope and matched each listing to its real video by the listing UUID embedded in that video's own description (ground truth, not title/timestamp inference), so the embed now covers nearly the entire catalog rather than just new uploads. Reply to Google's compliance review sent the same day, with two real screencasts (upload process + on-site embed playing) hosted on Google Drive after YouTube's own daily upload quota blocked uploading the screencasts themselves as Unlisted videos; now awaiting Google's response. Also same day: Beverly Hills Car Club's dealer account applied and was approved; six dealer logos added (Beverly Hills Car Club, Sterling Motors, Vaughns Classic Cars, Zoom Classic Cars, Ohio Corvettes and Muscle Cars, Atomic Motors), same existing fetch-from-dealer-site-and-upload pattern; and a small secondary "Visit website" link added to every listing detail page's dealer sidebar (only shown when `dealer.website` is set) — prompted by confirming via GA4 that outbound dealer-website clicks are tracked (Enhanced Measurement) but were low-volume because the link previously only existed on the lower-traffic dealer profile page, not listing pages. Separately, no code changed but a full monetization audit was done: an AI-generated revenue-ideas document was fact-checked line-by-line against the live site (three "currently missing" claims were stale — dealer analytics, advertiser reporting, and price-drop alerts all already exist and work; two were genuine confirmed gaps — lead-gen referral buttons and a Verified Documentation badge), followed by reading every markdown doc in the repo, which surfaced that Stripe billing is entirely unwired (no package, no billing columns, no working checkout anywhere) despite being the project's own documented highest-leverage item, and that a detailed inspection-referral spec (`Vehicle-Inspection-Marketplace-Spec.md`) already existed unread. Kicked off a near-term affiliate-referral track (financing/insurance/shipping/inspection partners) that doesn't depend on Stripe being wired. See IMPLEMENTATION_STATUS.md for full detail.)
> Prior update 2026-08-05, continuing the 2026-08-04 session (nine threads: (1) dealer-application email sends now check the Resend SDK's `{error}` field instead of assuming success — see `app/api/admin/dealer-applications/route.ts`; (2) Classic Car Center of Fredericksburg fully onboarded — 9 listings imported from the dealer's own site with hotlinked images (same pattern as feed-synced dealers), `website`/`logo` set, `classiccarcenter.net` added to `next.config.ts`'s image allow-list; (3) Vaughns Classic Cars' `dealer_car_search` feed fixed end-to-end — the intake bridge (a separate Node service on the VPS, not in this repo) expected an exact filename the vendor's export didn't use, and the format's image-URL parsing assumed the wrong delimiter for real production data. The delimiter fix was generalized: `imagesDelimiter` no longer exists in `FeedFormatColumns` at all — image URLs are now extracted via regex directly rather than split on any assumed separator character, so this class of bug can't recur for a future dealer using a different separator. 56 real listings now live for Vaughns; (4) YouTube API video-upload quota increase (1,000/day) submitted via Google's compliance audit process, decision still pending; (5) the Vaughns filename fix from (3) was only a one-time patch — `dealer-feed-bridge/server.js` (VPS-only) still hardcoded one exact filename; generalized to pick whichever `.csv` is newest in a dealer's folder regardless of name, archiving superseded files for 7 days instead of deleting them, protecting every current and future `sftp_incoming` dealer from this class of bug; (6) TikTok's Content Posting API "Direct Post" audit — the real gate on public posting, separate from the general app-approval email that initially (and incorrectly) looked like the all-clear — was found never to have been submitted, and was submitted for real this session; also surfaced and backfilled 7 real TikTok posts from 2026-07-30 whose `tiktok_posted_at` had never been recorded (same class of bug already fixed for Instagram/YouTube on 2026-07-31); (7) both TikTok's and YouTube's known, externally-blocked failure modes (pending audit / daily quota cap) downgraded from `error` to `warn` in `lib/tiktok/postShort.ts` and `lib/youtube/postShort.ts` so the automated hourly retries stop paging Sentry on an already-tracked condition — YouTube's fix matches specific quota-related reason codes rather than downgrading its init-failure branch wholesale, since that branch also legitimately handles unrelated failures; (8) the Virginia car club calendar (carclubcouncil.com) fully processed — all 69 one-time dated events now in `/admin` → Events (previously 57 unreviewed); (9) Zoom Classic Cars' feed vendor (JTZ Enterprise) sent a real sample export revealing their column naming doesn't match any of the five hardcoded-required header columns (`REQUIRED_FEED_COLUMNS` in `dealer-feed-sync/route.ts`, shared across every `feed_format` rather than per-vendor configurable) — the first vendor where even the required columns differ, not just the optional ones; asked the vendor to rename columns rather than building code-side support against an unconfirmed generic sample. No schema changes beyond what's already reflected in `dealers`' existing columns. See IMPLEMENTATION_STATUS.md for full detail.)
> Prior update 2026-08-03 (fixed a broken listing-detail sitemap URL / 404 for one approved listing (`61fd40b8-c8a8-4a58-8f3e-d7a1c7aaf8be`, make `Classic`) whose `model` field was an empty string — a bad-import artifact, not a systemic issue (confirmed no other listing has this gap). The blank model produced a double-slash URL that 404'd and also caused the sitemap-health cron's own segment-counting check to flag the same listing as "missing from the sitemap" despite a (broken) URL existing for it. Fixed the data directly (`model` → `Glassic Replica`, run by Derek in the Supabase SQL editor) and hardened `app/sitemap.ts`'s listing-URL generation to skip any listing missing `make`/`model` going forward, mirroring the existing unrecognized-make filter a few lines below it. No schema or API-contract changes. See IMPLEMENTATION_STATUS.md for full detail.)
> Prior update 2026-07-31 (dealer Feed Sync gained a second, Dealer Car Search column-naming format alongside the existing Speed Digital one — new `dealers.feed_format` column, default `'speed_digital'`, admin-set only; see §3 `dealers`. Vaughns Classic Cars onboarded as the first dealer on this format, delivering via the existing `sftp_incoming` push protocol. Their export tool turned out to only support standard FTP (port 21) or SFTP (port 22), not the intake's existing dedicated port 2022 — added a second `ftpd` listener on port 21 to the VPS's SFTPGo instance, purely additive alongside the unchanged `sftpd` listener on 2022 every other dealer's feed depends on; currently plain FTP, pending confirmation from the vendor on FTPS support. No other schema or API-contract changes. See IMPLEMENTATION_STATUS.md for full infra detail.)
> Prior update 2026-07-30 (auto-generated listing Reels now also cross-post to YouTube as a Short, alongside the existing Facebook/Instagram posting — new `lib/youtube/postShort.ts`, called fire-and-forget from `app/api/video-pipeline/complete/route.ts` alongside the existing Facebook/Instagram calls, own try/catch so a YouTube failure can't affect the others. Unlike Facebook/Instagram (which fetch a video URL themselves), YouTube's API requires pushing the video bytes via its resumable upload protocol, so the callback route now downloads the rendered video and re-uploads it — added `export const maxDuration = 60` to that route for the extra time this takes. New `listings.youtube_posted_at` column. Video description includes the listing's own description text (`description_paragraphs` preferred over plain `description`), truncated to stay under YouTube's 5000-char limit, plus per-listing hashtags (make, make+model, and `hobby_segment` when set — though `hobby_segment` has no write path anywhere in the live app today and is `null` on every real listing checked) instead of a fixed generic set. Also added: a model-keyword vehicle-segment classifier (`#SuperCar`/`#ExoticCar`/`#MuscleCar`/`#SportsCar`, priority-ordered, most-specific-signal-wins) and a selective `body_style` hashtag (only for styles worth hashtagging — Convertible/Coupe/Roadster/Pickup Truck/Fastback/Station Wagon/Hardtop, skipping the generic 2-Door/4-Door/Sedan/SUV). Both are plain in-code lists in `lib/youtube/postShort.ts`, not DB-driven. Real listings upload as Public by default; live-verified against the real YouTube API with a Private test upload before enabling. See IMPLEMENTATION_STATUS.md for full detail, including a real near-miss where the Google Cloud OAuth setup was initially built in the same project as the site's real "Sign in with Google" login before being caught and rebuilt in an isolated project. **TikTok added the same day** — `lib/tiktok/postShort.ts`, same wiring pattern, new `listings.tiktok_posted_at` column. Initially built against `PULL_FROM_URL` assuming the video was hosted on `garagecherries.com`; corrected after finding (via VPS SSH) that the video pipeline actually uploads to Supabase Storage, a domain not verified with TikTok — rebuilt on `FILE_UPLOAD` (download bytes, push to TikTok) like YouTube, plus a `creator_info/query` call to confirm the requested `privacy_level` is actually available for the account before posting. TikTok's OAuth needed a real deployed callback route (no localhost redirect URIs allowed, unlike Google) — `app/api/tiktok/oauth/callback/route.ts`, temporary/bootstrap-only. Live-verified with a real post after temporarily setting the account to Private (unaudited apps can only post to Private accounts — confirmed via a real `unaudited_client_can_only_post_to_private_accounts` error before that); account switched back to Public after. Real listings default to `PUBLIC_TO_EVERYONE`).
> Prior update 2026-07-29 (dealer Feed Sync gained a third protocol, `sftp_incoming` — dealers can now have their inventory system *push* a file to an SFTP server we host (SFTPGo, on the existing InterServer VPS built for the video pipeline), instead of only the two existing pull-based options; deliberately distinct naming from the existing outbound `feed_protocol = 'sftp'` (we pull from the dealer), which this sits alongside unchanged. New `dealers` columns `feed_sftp_username`/`feed_sftp_provisioned_at`/`feed_sftp_last_received_at` — no password column, unlike the existing outbound-SFTP feature's plaintext `feed_password`; this direction's actual credential lives only in SFTPGo's own account store, shown to the dealer once at creation. New dedicated route `POST/DELETE /api/dealer/feed-sftp/provision` (not the generic `/api/dealer/settings`, which has no field allowlisting) handles provisioning; `syncDealerFeed()` gained a third fetch branch reading the file back from a new small VPS bridge service instead of pulling from anywhere, plus a CSV header-validation guard (applied to all three protocols) against reading a dealer's file mid-upload. New daily staleness cron `GET /api/cron/dealer-feed-staleness` mirrors the existing `sitemap-health` pattern. See IMPLEMENTATION_STATUS.md for full detail).
> Prior update 2026-07-23 (dealer Feed Sync gained SFTP support alongside the existing HTTPS pull, for dealers whose inventory platform (e.g. Dealer.com/DDC) only exports via FTP/SFTP rather than a hosted URL — new `dealers` columns `feed_protocol`/`feed_host`/`feed_port`/`feed_username`/`feed_password`/`feed_remote_path`, `syncDealerFeed()` branches on `feed_protocol` via a new `fetchViaSftp()` helper (`ssh2-sftp-client`), the cron query now includes SFTP-configured dealers even with no `feed_url`, and the dealer Settings UI gained a Feed Type dropdown; SFTP credentials are stored the same way the existing `feed_url` already stores a vendor access-key — plaintext, protected by RLS only, not separately encrypted. Adding `ssh2-sftp-client` broke the Vercel production build (Turbopack can't bundle one of `ssh2`'s files for ESM) — fixed by adding `ssh2`/`ssh2-sftp-client` to `next.config.ts`'s `serverExternalPackages`, same treatment `sharp` already gets for the same class of issue. Also fixed two real display bugs: the pricing page's 250th Promo banner and advertiser-section text had literal "October 31, 2026"/"July 31, 2026" hardcoded despite the page already fetching the real, admin-configurable promo dates for other copy on the same page — now uses those dates everywhere, with a `timeZone: 'UTC'` fix for a one-day-off display artifact; the homepage promo popup (`PromoModal.tsx`) had its own separate hardcoded `2026-12-31` literal driving both its display text and its actual show/hide logic, completely independent of the site's real promo settings — now fetches `/api/public/trial-days` like the advertiser signup page already does. Also added a "Shop"/"Shop Merch" link (nav + footer) to the new GarageCherries merch storefront on Fourthwall. See IMPLEMENTATION_STATUS.md for full detail).
> Prior update 2026-07-22 (pagination added to `/listings` and every `/cars` listings grid — new shared `components/Pagination.tsx`, real crawlable page-number links, exact total counts instead of current-page-length; sort added to `/listings` (Newest/Price Low-High/Price High-Low/Featured First); `$0`-priced listings now display "Call For Price" via a new `formatListingPrice()` in `lib/data.ts`, matching the convention this doc already documented at §3 `listings.price` but that was never actually implemented; and a real production Facebook rate-limit crash fixed (reported via Sentry on `POST /api/dealer/feed-sync`) by removing the synchronous Facebook post on feed-sync insert and replacing it with a new hourly `GET /api/cron/facebook-post-queue` that drains a small batch of not-yet-posted listings regardless of source, so nothing gets skipped, just spread out over time. No schema or API-contract changes in any of this — see IMPLEMENTATION_STATUS.md for full detail).
> Prior update 2026-07-22 (dealer data-feed sync built end-to-end. New `dealer_locations` table + `GET/POST/PATCH/DELETE /api/dealer/locations` let a dealer with more than one physical showroom (e.g. Survivor Classic Car Services — Tampa/Chicago/Atlanta) manage each one from Settings; marking a location primary mirrors it onto the dealer's own row so every existing page reading those fields keeps working unchanged, and a dealer's public page shows an "Our Locations" section once 2+ exist. New `app/api/cron/dealer-feed-sync/route.ts` fetches a dealer's CSV inventory feed and three-way syncs it (insert/update/mark-sold, matched by VIN falling back to a new `stock_number` column), parsing per-row location data for vendors whose feed encodes it as a "City, State" label rather than real City/State columns, capping images at 30 evenly sampled across the full set, and flagging (not dropping) any make not yet in `MAKES`. Self-service added the same day: dealers configure their own `feed_url` and a `feed_sync_hour` (UTC) from a new Feed Sync settings section, and a "Sync now" button (`POST /api/dealer/feed-sync`) runs an immediate one-off sync sharing the exact same code the daily cron uses — the cron itself moved from a single hardcoded daily entry to running hourly and querying `dealers` for whichever ones' `feed_sync_hour` matches the current hour, so each dealer still gets one automatic sync/day but at a time they choose (required upgrading from Vercel's Hobby to Pro plan, which only allows once-daily cron on Hobby). Also fixed a real bug found while wiring this up: `is_feed_managed` (added 2026-07-02 as forward-compat for this exact feature) was never actually being set by the sync — now set true on both insert and update, so feed-managed listings correctly skip the manual renewal flow and expiring-listings reminder email as originally designed. See IMPLEMENTATION_STATUS.md for full detail).
...
| `sold_at` | timestamptz \| null | Set correctly by the manual dealer-facing "mark as sold" route (`POST /api/cars/sold`) since it was built. `dealer-feed-sync`'s auto-detected sales (a listing disappearing from a dealer's feed) set `is_sold` but left this null until fixed 2026-07-31 — confirmed all 4 then-sold listings had it null, backfilled to the fix date since no reliable historical sale date existed. Used by `admin/overview`'s "sold in last 30 days" stat and the public `/sold` page's sort order, both of which silently undercounted/misordered before the fix. |
...
| `youtube_posted_at` | timestamptz \| null | Added 2026-07-30. Set when the listing's auto-generated Reel is successfully uploaded to YouTube (as a Short); mirrors `reel_posted_at`, kept separate since it's an independent platform with its own success/failure outcome. Was fire-and-forget (not awaited) in `video-pipeline/complete` until 2026-07-31, so a real successful upload frequently never got this column stamped — confirmed against production (0 rows set despite 14 real videos existing); now awaited, same as `reel_posted_at`. The 14 pre-existing real uploads were manually matched to their listings and backfilled via SQL after the fix. |
| `tiktok_posted_at` | timestamptz \| null | Added 2026-07-30. Set when the listing's auto-generated Reel is successfully posted to TikTok; same independent-outcome reasoning as `youtube_posted_at`. Same fire-and-forget gap as `youtube_posted_at` until 2026-07-31, now awaited. Also blocked independently by TikTok's own unaudited-app restriction (see `lib/tiktok/postShort.ts` — `postListingReelToTikTok()` requests `PUBLIC_TO_EVERYONE` by default, which this app cannot currently post with; only `FOLLOWER_OF_CREATOR`/`MUTUAL_FOLLOW_FRIENDS`/`SELF_ONLY` are available pending TikTok's app review). Optional env var `TIKTOK_DEMO_PRIVACY_LEVEL` overrides the requested privacy level (defaults to unchanged `PUBLIC_TO_EVERYONE`) — added solely to record the app-review demo video against a real successful post; should not be set in normal production operation. |
| `instagram_posted_at` | timestamptz \| null | Added 2026-07-30. Instagram posting existed since the video pipeline was built but was fire-and-forget with no outcome ever recorded; added so `video-pipeline/complete` can check "already posted?" per platform independently instead of always re-attempting Instagram on every re-render. Remained fire-and-forget (not actually awaited) until 2026-07-31 despite the column's addition — confirmed against production (0 rows set despite 243 real videos existing). 214 of the ~243 pre-existing real posts were manually matched to their listings (screenshot transcription cross-referenced against the DB) and backfilled via SQL; the remainder were either sold listings excluded from future auto-processing anyway, or not conclusively matchable and left for the automated backfill to catch naturally. |
| `youtube_video_id` | text \| null | Added 2026-08-07. The real YouTube video ID returned by `videos.insert` on a successful upload, captured by `postListingReelToYouTube()` (previously discarded — only the boolean success/failure was returned) and persisted in `video-pipeline/complete`. Drives the on-site embed: `app/listings/[...segments]/page.tsx` renders `components/YouTubeEmbed.tsx` (click-to-play, `youtube-nocookie.com`) only when this column is set. Built in response to Google's YouTube API compliance review asking for evidence of an on-site embedded player, which didn't exist until now. Confirmed populated and embedding correctly the same day against real production listings (`1953 Ford F100 Pickup Truck`, `1990 Buick Reatta Convertible`). Backfilled the same day for 121 of 122 pre-existing listings that had a real upload predating this column, matched via the listing UUID embedded in each video's own description rather than title/timestamp guessing; 1 listing had no matching video on the channel and remains unbackfilled. |
...
| `feed_url` | text \| null | Added 2026-07-22. Dealer-configured CSV data-feed URL (Settings → Feed Sync); when set, this dealer is eligible for `GET /api/cron/dealer-feed-sync` and `POST /api/dealer/feed-sync` (on-demand "Sync now"). Fetched with a real browser-like `User-Agent` header as of 2026-09-08 (previously sent none, which meant Node's default `User-Agent: node` went out on every request) — added after Garage Kept Motors, LLC's Cloudflare-fronted feed returned a 403 to the sync's bare fetch while the identical request succeeded from a non-Vercel origin. |
...
| `feed_sftp_username` | text \| null | Added 2026-07-29. Set when `feed_protocol = 'sftp_incoming'` (dealer pushes a file to an SFTP server we host, the reverse of the outbound `'sftp'` protocol above) — the SFTPGo virtual-user username assigned at provisioning. No corresponding password column: unlike the outbound-SFTP fields above, this direction's credential is never stored here, only in SFTPGo's own account store on the VPS. |
...
| Dealer | Feed Protocol | Format | Listings | Status |
|---|---|---|---|---|
| RK Motors LLC | `https` (we pull, hourly at :01 UTC) | `speed_digital` | 63 | Live, syncing daily |
| Survivor Classic Car Services | `https` (we pull, hourly at :01 UTC) | `speed_digital` | 90 | Live, syncing daily |
| Ohio Corvettes and Muscle Cars | `https` (we pull, hourly at :01 UTC) | `speed_digital` | 84 | Live, syncing daily |
| Vaughns Classic Cars | `sftp_incoming` (we host, Dealer Car Search pushes) | `dealer_car_search` | 56 | **Live as of 2026-08-04** — filename mismatch on the VPS bridge and an image-parsing delimiter bug both found and fixed this session; `feed_sync_hour` = 7 UTC. See `IMPLEMENTATION_STATUS.md` 2026-08-04 entry for the full story. |
| McGinty Motorcars | `sftp_incoming` (we host, DDC/Dealer.com pushes) | `speed_digital` | 0 | Provisioned (2026-07-30) but `feed_sync_hour` is still unset and no file has ever been received — needs DDC's actual export schedule (or a placeholder hour) before it can sync even once they start pushing |
| Classic Car Center of Fredericksburg | None (manual import) | `speed_digital` (unused default) | 9 | No automated feed — Marty's own site has no feed export; 9 listings manually imported this session from classiccarcenter.net with hotlinked images, same technique used for feed-synced dealers |
| Platt Motors Inc | None configured | `speed_digital` (unused default) | 2 | Manual listing management only. **No SFTP account provisioned at all** — needs someone with Platt's dealer login to click "Generate Credentials" before their vendor can be given real connection details |
| AutoArcheologist | None configured | `speed_digital` (unused default) | 25 | Manual listing management only |
| Sterling Motors Inc. | None configured | `speed_digital` (unused default) | 12 | Manual listing management only |
| Zoom Classic Cars, LLC | None configured | `speed_digital` (unused default) | 0 | Manual listing management only — data feed setup status with their provider (JTZ Enterprise) unknown, no confirmation received |
...
Added 2026-08-08 for the Independent Inspection Reports feature (`PRODUCT_OVERVIEW.md`'s "outside-upload" design). One-to-one with `listings` via a unique `listing_id`. RLS: public read (`using (true)`); no write policies — all writes go through `POST/DELETE /api/dealer/inspection`, which uses the service-role admin client after verifying the listing's `seller_id` matches the authenticated dealer.

| Field | Type | Notes |
|---|---|---|
| `id` | uuid | PK, `gen_random_uuid()` |
| `listing_id` | text | FK to `listings.id`, `ON DELETE CASCADE`, `UNIQUE` (enforces 1:1) |
| `provider_name` | text | e.g. "Lemon Squad" |
| `report_date` | date \| null | Displayed with `{ timeZone: 'UTC' }` to avoid the same off-by-one-day bug fixed for promo dates on 2026-07-23 |
| `summary` | text \| null | |
| `report_url` | text | PDF, uploaded to the `inspection-reports` Storage bucket (public) |
| `photo_urls` | text[] | Optional additional inspection photos, same bucket |
| `created_at` / `updated_at` | timestamptz | |

Dealer-side upload/edit/remove UI: `components/InspectionReportSection.tsx`, mounted in the Edit Vehicle modal (`app/dealer/dashboard/page.tsx`) only in edit mode. Buyer-facing summary card: `components/InspectionReportCard.tsx`, rendered on the listing detail page when a report exists, with a disclaimer ("as-is," not commissioned/verified/endorsed by GarageCherries, buyer solely responsible for independent verification) mirrored onto `/terms` (`app/terms/page.tsx`, same hardcoded-section-below-Enzuzo pattern as `/privacy`'s YouTube API Services section) — see `Legal-Review-Checklist.md` item #23, still flagged for attorney review.
...
### `POST/DELETE /api/dealer/feed-sftp/provision` (added 2026-07-29)
- **Auth**: required, must own dealer record — checked directly here rather than via the generic `/api/dealer/settings`, which has no field allowlisting and isn't used for this
- **`POST` behavior**: calls the VPS bridge to create an isolated SFTPGo virtual user for this dealer (home-jailed to their own upload directory), then sets `feed_protocol = 'sftp_incoming'` and stores `feed_sftp_username`/`feed_sftp_provisioned_at` on the `dealers` row. The password is generated by the bridge and returned only in this response — never stored in our database.
- **`POST` returns**: `{ username, password, host, port }` — a one-time credential display; re-calling `POST` again generates a fresh password (the old one stops working)
- **`DELETE` behavior**: calls the bridge to remove the SFTPGo account and its uploaded files, then clears `feed_protocol`/`feed_sftp_username`/`feed_sftp_provisioned_at`/`feed_sftp_last_received_at` on the `dealers` row
