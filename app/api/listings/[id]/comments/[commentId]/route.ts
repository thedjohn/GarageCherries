...
// The comment's own author, or the listing's seller/team, can delete it.
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string; commentId: string }> }) {
  ...
  const canDelete = comment.author_id === user.id || await isAuthorizedForSeller(user.id, sellerId);
