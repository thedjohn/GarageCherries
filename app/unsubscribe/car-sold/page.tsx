...
export default async function UnsubscribeCarSoldPage({
  searchParams,
}: {
  ...
  if (uid && UUID_RE.test(uid)) {
    ...
    if (error || !user) {
      invalid = true;
    } else if (user.user_metadata?.car_sold_opt_out) {
      alreadyDone = true;
    } else {
      await admin.auth.admin.updateUserById(uid, {
        user_metadata: { ...user.user_metadata, car_sold_opt_out: true },
      });
      success = true;
    }
  } else {
          ...
          {success || alreadyDone ? (
            <>
              ...
              <p className="text-sm text-zinc-500 mb-6">
                You won't receive any more &quot;car sold&quot; notifications from GarageCherries.
