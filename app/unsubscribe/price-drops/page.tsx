...
export default async function UnsubscribePriceDropsPage({
  searchParams,
}: {
  ...
  if (uid && UUID_RE.test(uid)) {
    ...
    if (error || !user) {
      invalid = true;
    } else if (user.user_metadata?.price_drop_opt_out) {
      alreadyDone = true;
    } else {
      await admin.auth.admin.updateUserById(uid, {
        user_metadata: { ...user.user_metadata, price_drop_opt_out: true },
      });
      success = true;
    }
  } else {
          ...
          {success || alreadyDone ? (
            <>
              ...
              <p className="text-sm text-zinc-500 mb-6">
                You won't receive any more price drop notifications from GarageCherries.
