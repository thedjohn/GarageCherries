...
export default async function Decade1980sPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  const { page } = await searchParams;
  return <DecadePage content={getDecadeContent('1980s')!} page={Math.max(1, parseInt(page ?? '1', 10) || 1)} />;
}
