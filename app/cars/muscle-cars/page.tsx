...
export const metadata: Metadata = {
  title: 'Muscle Cars For Sale — History, Notable Models & Buying Guide',
  description: 'Shop muscle cars for sale from trusted dealers and private sellers — Camaro, Chevelle, GTO, Charger, Mustang, Road Runner, and more. Plus history, notable models, and buying tips.',
  alternates: { canonical: 'https://www.garagecherries.com/cars/muscle-cars' },
};

export default async function MuscleCarsPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  // same logic as the /cars encyclopedia's matchModel prefix matching.
  const filterByMake = new Map(MUSCLE_CAR_FILTER.map(f => [f.make, f.modelPrefixes]));
  const allMatching = (dbRows ?? []).filter(r => {
    const prefixes = filterByMake.get(r.make);
    if (!prefixes || !r.model) return false;
    const modelLower = r.model.toLowerCase();
    return prefixes.some(p => modelLower.startsWith(p.toLowerCase()));
  });

  const totalCount = allMatching.length;
  const totalPages = Math.max(1, Math.ceil(totalCount / PAGE_SIZE));
  const pageRows = allMatching.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: content.title, item: `https://www.garagecherries.com/cars/${content.slug}` },
    ],
  };
    ...
    <>
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        <nav className="flex items-center gap-2 text-sm text-zinc-400 mb-8">
          ...
          <span className="text-zinc-700">{content.title}</span>
        </nav>

        <h1 className="text-3xl font-extrabold text-zinc-900 mb-2">{content.title}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-12">
          {content.notableModels.map(model => (
            <div key={model.name} className="bg-white border border-zinc-100 rounded-2xl p-6">
              <div className="flex items-baseline gap-3 flex-wrap">
                <h3 className="text-lg font-bold text-zinc-900">{model.name}</h3>
                <span className="text-xs text-zinc-400">{model.years}</span>
              </div>
              <p className="text-sm text-zinc-500 mt-2">{model.blurb}</p>
            </div>
          ))}
        </div>
        ...
        <h2 className="text-xl font-bold text-zinc-900 mb-5">
          Muscle Car Listings {totalCount > 0 && <span className="text-zinc-400 font-normal">({totalCount})</span>}
        </h2>
        {totalCount === 0 ? (
          <p className="text-zinc-400 text-sm mb-8">
            No muscle car listings right now — <Link href="/listings" className="text-red-600 hover:underline">browse all cars for sale</Link>.
          </p>
        ) : (
          <div className="mb-8">
