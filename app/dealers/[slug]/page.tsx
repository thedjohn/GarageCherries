...
import { getDealer, CARS } from '@/lib/data';
...
export default async function DealerPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const addressParts = [dealer.address, dealer.location, dealer.state, dealer.zip].filter(Boolean);
  ...
  const jsonLd = {
    ...
    address: {
      ...
      ...(dealer.zip ? { postalCode: dealer.zip } : {}),
      addressCountry: 'US',
    },
  };
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              <div>
                ...
                <div className="flex items-center gap-2 text-zinc-500 text-sm mt-1">
                  ...
                  {dealer.location}, {dealer.state} · Est. {dealer.since}
                </div>
              </div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-2xl font-bold text-zinc-900">
          Current Inventory
          <span className="ml-2 text-lg font-normal text-zinc-400">({allListings.length})</span>
        </h2>
      </div>
