...
const log = createLogger('lib/indexNow');
...
// triggered it, so this never throws. It does now log failures (including
// non-2xx responses, which `fetch` doesn't treat as an error on its own)
// so a silent submission failure is actually visible instead of vanishing.
