...
*Updated 2026-08-18 — a silent-undercounting bug (Supabase/PostgREST's default 1000-row cap on any unbounded `.select()`, silently truncating results computed in JS instead of via `count()`/`.range()`) was found and fixed across six spots: the homepage Active Listings stat, the admin panel's Overview tab pending-queue count/oldest-pending-date, the `/dealers` directory's per-dealer listing counts, the admin Users tab's per-user listing/watchlist/conversation counts, `/cars/muscle-cars`, and `/reports`. New/updated items in §1 and §6 below.*
...
- [ ] Dealer directory (`/dealers`) lists all dealers with listing counts — each comes from a real per-dealer count query now, not a fetch-all-and-group approach that would silently undercount once site-wide active listings crossed 1000 (fixed 2026-08-18); spot-check a high-volume dealer (e.g. Beverly Hills Car Club) shows its true count
...
- [ ] `/cars/muscle-cars` (added 2026-08-08) loads — history, notable models, buying tips, and real matching listings (should show 80+ real cars: Camaro, Chevelle, GTO, Charger, Challenger, Mustang, etc.); pagination works past page 1; "Featured Guide" card appears on `/cars` above the SRT card; Footer "Muscle Cars" link (Browse column) points to this page; confirm `/sitemap.xml` includes it; matching listings are fetched in full regardless of Supabase's 1000-row query cap (paging fix added 2026-08-18)
- [ ] Buyer's Guides (`/guides`) — index and at least one article open
- [ ] Market Report (`/reports`) loads with live data — averages/median/counts are computed from every active listing, not just the first 1000 (paging fix added 2026-08-18)
...
- [ ] **Overview tab**: shows a real pending-queue count and "Oldest submitted Nd ago" line — both come from targeted count/oldest-row queries now, not by fetching every pending row into JS and computing `.length`/`Math.max()` over it, which would silently under/miscount once the pending queue crossed 1000 (fixed 2026-08-18)
...
- [ ] **Users tab**: search/filter users, view a seller's listings; a high-volume account's own listing/watchlist/conversation counts stay accurate past Supabase's 1000-row query cap (fixed 2026-08-18 via a new `fetchAllRows()` paging helper) — spot-check Beverly Hills Car Club (`joe harwick`) shows 533 active listings, matching a direct database count
